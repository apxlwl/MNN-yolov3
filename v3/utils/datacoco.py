...
class Data(object):
    def __init__(self):
        ...
        """
        self.__dataset_path = cfg.COCO_PATH
        ...
        annotations = self.__load_annotations(self.__dataset_path)
    ...
    def __load_annotations(self, data_path, use_difficult_bbox=False):
        ...
        """
        img_inds_file = os.path.join(data_path, 'ImageSets','train.txt')
        with file(img_inds_file, 'r') as f:
            txt = f.readlines()
            image_inds = [line.strip() for line in txt]
        annotations = []
        # image_inds=image_inds[:10000]
        for image_ind in image_inds:
            image_path = os.path.join(data_path, 'images','train2017', image_ind + '.jpg')
            annotation = image_path
            label_path = os.path.join(data_path, 'voc_annotations', image_ind + '.xml')
...
if __name__ == '__main__':
    ...
    data_obj = Data
    valset = tf.data.Dataset.from_generator(data_obj,
                                            ((tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32,
                                              tf.float32)))
    valset = valset.batch(1, drop_remainder=True).prefetch(2)
    iterator = valset.make_one_shot_iterator()
    one_element = iterator.get_next()
    with tf.Session() as sess:
    #
        for _ in tqdm(range(500)):
            batch_image, batch_label_sbbox, batch_label_mbbox, batch_label_lbbox, \
            batch_sbboxes, batch_mbboxes, batch_lbboxes=sess.run(one_element)
    #     # print batch_image.shape, batch_image.dtype
    #     # print batch_label_sbbox.shape, batch_label_sbbox.dtype
    #     # print batch_label_mbbox.shape, batch_label_mbbox.dtype
    #     # print batch_label_lbbox.shape, batch_label_lbbox.dtype
    #     # print batch_sbboxes.shape, batch_sbboxes.dtype
    #     # print batch_mbboxes.shape, batch_mbboxes.dtype
    #     # print batch_lbboxes.shape, batch_lbboxes.dtype
    #     # assert 0
    valset=Data()
    for idx,(batch_image, batch_label_sbbox, batch_label_mbbox, batch_label_lbbox, \
                       batch_sbboxes, batch_mbboxes, batch_lbboxes )in tqdm(enumerate(valset)):
        if idx==500:
            break
        # print batch_image.shape,batch_image.dtype
        # print batch_label_sbbox.shape,batch_label_sbbox.dtype
